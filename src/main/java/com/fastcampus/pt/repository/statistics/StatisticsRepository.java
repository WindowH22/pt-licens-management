package com.fastcampus.pt.repository.statistics;
...
public interface StatisticsRepository extends JpaRepository<StatisticsEntity,Integer> {
    @Query(value = "SELECT new com.fastcampus.pt.repository.statistics.AggregatedStatistics(s.statisticsAt, SUM(s.allCount), SUM(s.attendedCount), SUM(s.cancelledCount)) " +
